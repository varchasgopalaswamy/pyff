...
class DirectoryPyfference:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        self,
        packages: pp.PackagesPyfference | None,
        modules: pm.ModulesPyfference | None,
    ) -> None:
        self.packages: pp.PackagesPyfference | None = packages
    ...
    while traverse_directories:
        ...
        for item in candidate.iterdir():
            if item.is_file() and item.suffix == ".py":
                ...
                LOGGER.debug(
                    "Item %s is not anything Python-related, so we do not care",
                    str(item),
                )
...
def _compare_packages_in_dir(
    ...
    changed_packages = {
        ...
        for pkg, change in [
            (
                pkg,
                pp.pyff_package(
                    pp.summarize_package(old_dir / pkg),
    ...
    new_package_summaries = {
        pkg: pp.summarize_package(new_dir / pkg) for pkg in new_packages
    }
...
def _compare_modules_in_dir(
    ...
    removed_module_summaries = {
        mod: pm.summarize_module(old_dir / mod) for mod in removed_modules
    }
    changed_modules = {
        mod: change
        for mod, change in [
            (
                mod,
                pm.pyff_module(
                    pm.summarize_module(old_dir / mod),
                    pm.summarize_module(new_dir / mod),
                ),
            )
    ...
    new_module_summaries = {
        mod: pm.summarize_module(new_dir / mod) for mod in new_modules
    }

    if removed_module_summaries or changed_modules or new_module_summaries:
        return pm.ModulesPyfference(
            removed_module_summaries, changed_modules, new_module_summaries
        )

    return None


def pyff_directory(old: pathlib.Path, new: pathlib.Path) -> DirectoryPyfference | None:
    """Find Python packages and modules in two directories and compare them"""
    if not (old.is_dir() and new.is_dir()):
        msg = f"At least one of {old}, {new} is not an existing directory"
        raise ValueError(msg)

    old_pkgs, old_mods = find_those_pythonz(old)
    new_pkgs, new_mods = find_those_pythonz(new)

    packages: pp.PackagesPyfference | None = _compare_packages_in_dir(
        old, new, old_pkgs, new_pkgs
    )
    modules: pm.ModulesPyfference | None = _compare_modules_in_dir(
        old, new, old_mods, new_mods
    )
