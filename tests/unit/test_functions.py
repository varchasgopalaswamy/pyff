...
class TestExternalUsageChange:
    def test_sanity(self):
        euc = pf.ExternalUsageChange(
            gone={"name", "another_name"}, appeared={"new_name"}
        )
    ...
    def test_equality(self):
        ...
        euc_same = pf.ExternalUsageChange(
            gone={"another_name", "name"}, appeared={"new_name"}
        )
...
class TestFunctionPyfference:
    def test_sanity(self):  # pylint: disable=invalid-name
        fic = pf.FunctionImplementationChange()
        fp1 = pf.FunctionPyfference(
            name="function", implementation={fic}, old_name="old_function"
        )
...
class TestCompareImportUsage:
    def test_no_external(self):
        ...
        assert (
            pf.compare_import_usage(
                old_function, new_function, old_imports, new_imports
            )
            is None
        )
    ...
    def test_appeared(self):
        ...
        change = pf.compare_import_usage(
            old_function, new_function, old_imports, new_imports
        )
...
class TestPyffFunction:
    ...
    def test_identical(self):
        ...
        assert (
            pf.pyff_function(old, new, pi.ImportedNames(), pi.ImportedNames()) is None
        )
...
class TestPyffFunctionCode:
    ...
    def test_invalid(self):
        with pytest.raises(ValueError):
            pf.pyff_function_code(
                self.FUNCTION, self.KLASS, pi.ImportedNames(), pi.ImportedNames()
            )

        with pytest.raises(ValueError):
            pf.pyff_function_code(
                self.KLASS, self.FUNKTION, pi.ImportedNames(), pi.ImportedNames()
            )
...
class TestFunctionsPyfference:
    def test_sanity(self):
        ...
        assert change.new["function"] == pf.FunctionSummary(
            "function", node=mocked_node
        )
        assert change.new["funktion"] == pf.FunctionSummary(
            "funktion", node=mocked_node
        )
